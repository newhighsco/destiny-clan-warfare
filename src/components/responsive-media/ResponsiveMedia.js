...
const ResponsiveMedia = class extends PureComponent {
  render() {
    ...
    const ratioPercentageRounded =
      ratioPercentage && parseFloat(ratioPercentage.toFixed(4))
...
ResponsiveMedia.propTypes = {
  ...
  ratio: (props, propName, componentName) => {
    ...
    return !ratio || ratioRegex.exec(ratio)
      ? null
      : new Error(
          `${propName} (${
            props[propName]
          }) in ${componentName} is not in the expected format (e.g. 16:9)`
        )
  },
